...
class TextField with Destroyable {
  ...
  /// returned.
  Future<Optional<String>> display() async {
    _redisplay();
    ...
    return inputLoop.listen((key) {
      ...
      } else if (key.controlChar == ControlCharacter.ctrlJ) {
        // Ctrl + Enter
        _text += '\n';
      } else if (key.controlChar == ControlCharacter.ctrlH) {
        // Ctrl + Backspace
        var match = WORD_BACKSPACE.firstMatch(_text);
        if ((match?.groupCount ?? 0) == 2) {
          _text = _text.substring(0, match.group(1).length);
        } else if (_text.isNotEmpty) {
          _text = '';
        }
      } else if (!key.isControl &&
          (maxLength == -1 || _text.length <= maxLength)) {
        _text += key.char;
      }

      _redisplay();
      return true;
    }, breakOn: [ControlCharacter.enter]).componentValue(() => _text);
  }
